...
class Plumrocket_SocialLogin_Helper_Data extends Plumrocket_SocialLogin_Helper_Main
{
    ...
    const LOG_FILE_NAME = 'pslogin.log';
    ...
    /**
	 * Retrieve boolean true when customer email begins with the FAKE_EMAIL_PREFIX
	 * You can use second parameter useOrigData in case when you need
	 * checking only original customer email address
	 * For example:
	 * In some cases getEmail and getOrigData can returning different values
	 *
	 * @param  null|string  $email
	 * @param  boolean $useOrigData
	 * @return boolean
	 */
    public function isFakeMail($email = null, $useOrigData = false)
    {
        if (is_null($email)) {
            $session = Mage::getSingleton('customer/session');
    ...
    {
        return Mage::getStoreConfig('pslogin/developer/enable');
    }
