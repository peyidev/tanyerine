...
class Plumrocket_SocialLogin_Model_Account extends Mage_Core_Model_Abstract
{
    ...
    public function _setLog($data, $append = false, $isCallRequest = false)
    {
        if (Mage::helper('pslogin')->getDebugMode ()) {
            ...
            $log[] = '---------------------------------';
            $custumerSession = Mage::getSingleton('customer/session');
            $psloginLog = $custumerSession->getPsloginLog();
            if (null === $psloginLog) {
                $psloginLog = array();
            }
            $psloginLog[] = implode(PHP_EOL, $log);
            $custumerSession->setPsloginLog($psloginLog);
        }
    }
    ...
    {
        $file = Plumrocket_SocialLogin_Helper_Data::LOG_FILE_NAME;
        Mage::log(implode(PHP_EOL, Mage::getSingleton('customer/session')->getPsloginLog()), null, $file, true);
    }
