...
class J2t_Rewardpoints_Model_Total_Points extends J2t_Rewardpoints_Model_Total_Points_Abstract {
    ...
    const XML_PATH_DISABLE_POINTS_DISCOUNT = 'rewardpoints/default/disable_points_discount';
	...
	if(!$creditPoints)
        ...
        $diable_points_coupon = Mage::getStoreConfig(self::XML_PATH_DISABLE_POINTS_DISCOUNT, $address->getQuote()->getStoreId());
        if ($creditPoints && $diable_points_coupon) {
            //check if products within cart has special price or is discounted
            $verify = true;
            foreach ($items as $item) {
                if ($verify){
                    ...
                    $specialprice_verification = false;
                    if ($specialprice && ($product->getPrice()>$product->getFinalPrice())){
                        if($today >= strtotime( $specialPriceFromDate) && $today <= strtotime($specialPriceToDate) || 
                            $today >= strtotime( $specialPriceFromDate) && is_null($specialPriceToDate)){
                                //product in sale
                                $specialprice_verification = true;
                        }
                    }
                    if ($item->getBaseDiscountAmount() || $specialprice_verification 
                            || (float)$item->getProduct()->getPrice() > (float)$item->getBaseOriginalPrice()){
                        $creditPoints = 0;
                        Mage::helper('rewardpoints/event')->setCreditPoints(0);

                        $address->getQuote()
                                ->setRewardpointsQuantity(0)
                                ->setBaseRewardpoints(0)
                                ->setRewardpoints(0);
                        Mage::getSingleton('core/session')->addError('You cart currently contains discounted products. Therefore, points usage is not allowed.'); 
                        $verify = false;
                    }
                }
            }
        ...
        if ($creditPoints > 0 && $this->checkMinUse($address->getQuote())) {
            ...
            foreach ($items as $item) {
                ...
                if ($max_usage = $product->getData('reward_points_limit_usage')) {
                    ...
                    $baseDiscountAmount = min($baseDiscountAmount, ($max_usage * $qty));
                }
        ...
        if (is_object($address) && is_object($address->getQuote())) {
            $quote_order = $address->getQuote();
        }
