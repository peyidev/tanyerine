...
class J2t_Rewardpoints_Model_Discount extends Mage_Core_Model_Abstract
{
	...
	if ($processTax){
	    //$tax = $this->_quote->getShippingAddress()->getBaseTaxAmount();
            ...
            $subtotalPrice = ($subtotalPrice > 0) ? $subtotalPrice : $this->_quote->getBillingAddress()->getBaseSubtotalInclTax();
	} else {
            $subtotalPrice = $this->_quote->getShippingAddress()->getBaseSubtotal();
            $subtotalPrice = ($subtotalPrice > 0) ? $subtotalPrice : $this->_quote->getBillingAddress()->getBaseSubtotal();
        }
	...
	if (version_compare(Mage::getVersion(), '1.4.0', '>=') && $shipping_process){
            if ($processTax){
                ...
                $shipping = ($shipping > 0) ? $shipping : $this->_quote->getBillingAddress()->getBaseShippingInclTax();
            } else {
                $shipping = $this->_quote->getShippingAddress()->getBaseShippingAmount();
                $shipping = ($shipping > 0) ? $shipping : $this->_quote->getBillingAddress()->getBaseShippingAmount();
            }
