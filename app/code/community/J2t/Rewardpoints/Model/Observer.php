...
class J2t_Rewardpoints_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function discountBefore(Varien_Event_Observer $observer) {
        $quote = $observer->getEvent()->getQuote();
        $result = $observer->getEvent()->getResult();
        $item = $observer->getEvent()->getItem();
        $discountAmount = $result->getDiscountAmount();
        $baseDiscountAmount = $result->getBaseDiscountAmount();
        $item->setDiscountBeforeRewards($item->getDiscountAmount()+$discountAmount);
        $item->setBaseDiscountBeforeRewards($item->getBaseDiscountAmount()+$baseDiscountAmount);
    }
    

    public function discountVerification(Varien_Event_Observer $observer) {
        /** @var $quote Mage_Sales_Model_Quote */
        $quote = $observer->getEvent()->getQuote();

        return $this;
        
        if ($quotePoints = $quote->getRewardpointsQuantity()) {
            $quote->setVoidRewardsTotal(true);
            $items = $quote->getAllItems();
            $accBaseDiscount = 0;
            $accDiscount = 0;
            $points_to_get_money = Mage::getStoreConfig('rewardpoints/default/points_money', $quote->getStoreId());
            foreach ($items as $item){
                if ($pointsUsed = $item->getRewardpointsUsed()){
                    $baseDiscountAmount = Mage::helper('rewardpoints/data')->processMathValueCart(($pointsUsed / $points_to_get_money), null, true);
                    $discountAmount = $quote->getStore()->convertPrice($baseDiscountAmount, false);
                    
                    if (($discountAmount + $item->getDiscountBeforeRewards()) != $item->getDiscountAmount()) {
                        $accBaseDiscount += $baseDiscountAmount;
                        $accDiscount += $discountAmount;
                        $item->setDiscountAmount($discountAmount + $item->getDiscountAmount());
                        $item->setBaseDiscountAmount($baseDiscountAmount + $item->getBaseDiscountAmount());
                    }
                }

            }
            
            if ($accBaseDiscount > 0 && $accDiscount > 0){
                //foreach ($quote->getAllAddresses() as $address) {
                    //echo $address->getBaseGrandTotal();
                $address = $quote->getBillingAddress();    
                    $address->addTotalAmount('discount', -$accDiscount);
                    $address->addBaseTotalAmount('discount', -$accBaseDiscount);
    ...
    public function addMassAction($observer) {
        ...
        if (get_class($block) == 'Mage_Adminhtml_Block_Customer' && $block->getRequest()->getControllerName() == 'customer') {
            ...
            $block->addButton('rewardpoints_flush_all', array(
                ...
                'onclick' => 'confirmSetLocation(\'' . $message . '\', \'' . $block->getUrl('*/rewardpointsadmin_clientpoints/deleteAllFromGrid') . '\')',
            ));
        }
    }
	...
	//inject coupon for VES_VendorsCheckout coupon
        if ($block instanceof VES_VendorsCheckout_Block_Cart_Coupon){
            if (version_compare(Mage::getVersion(), '1.5.0', '>=')) {
                $transport = $observer->getTransport();
                $html = $transport->getHtml();
            }
            ...
            $productsHtml->setTemplate('rewardpoints/reward_coupon.phtml');
            $productsHtml->setNameInLayout("checkout_cart_coupon_ves");
            $extraHtml = $productsHtml->toHtml();
        ...
        if (is_object(Mage::app()->getFrontController()) && is_object(Mage::app()->getFrontController()->getAction()) && (
                ...
                ) && Mage::helper('rewardpoints')->isModuleActive() && $block->getTemplate() != 'email/order/invoice/items.phtml') {
    ...
    public function recordRegistrationFromAdmin($observer) {
        $this->recordPointsUponRegistration($observer);
    }
    
    public function recordPointsUponRegistration($observer) {
        $customerId = $observer->getEvent()->getCustomer()->getEntityId();

        if (Mage::helper('rewardpoints')->isModuleActive($observer->getEvent()->getCustomer()->getStoreId(), $observer->getEvent()->getCustomer()->getGroupId())) {
            //check referral parent
            $referrer = Mage::getModel('rewardpoints/referral')->loadByEmail($observer->getEvent()->getCustomer()->getEmail());
            if ($referrer->getId()) {
                Mage::getSingleton('rewardpoints/session')->setReferralUser($referrer->getRewardpointsReferralParentId());
            }
            if (Mage::getStoreConfig('rewardpoints/registration/registration_points', $observer->getEvent()->getCustomer()->getStoreId()) > 0) {
                //check if points already earned
                $points = Mage::getStoreConfig('rewardpoints/registration/registration_points', $observer->getEvent()->getCustomer()->getStoreId());
                $min_customer_id = Mage::getStoreConfig('rewardpoints/registration/registration_points_customer_id', $observer->getEvent()->getCustomer()->getStoreId());
                if ((int) $min_customer_id < $customerId) {
                    $this->recordPoints($points, $customerId, J2t_Rewardpoints_Model_Stats::TYPE_POINTS_REGISTRATION, null, false, false, false, $observer->getEvent()->getCustomer()->getStoreId());
                }
            }
            ...
            if ($customer->getId() != $referrer_id && $customer->getId() && $referrer_id && ($points = Mage::getStoreConfig('rewardpoints/registration/referrer_registration_points', $observer->getEvent()->getCustomer()->getStoreId()))) {
                $this->recordPoints($points, $referrer_id, J2t_Rewardpoints_Model_Stats::TYPE_POINTS_REFERRAL_REGISTRATION, null, false, $customerId, false, $observer->getEvent()->getCustomer()->getStoreId());
            }
            if ($customer->getId() && $referrer_id && ($points = Mage::getStoreConfig('rewardpoints/registration/referred_registration_points', $observer->getEvent()->getCustomer()->getStoreId()))) {
                $this->recordPoints($points, $customerId, J2t_Rewardpoints_Model_Stats::TYPE_POINTS_REFERRAL_REGISTRATION, null, false, $referrer_id, false, $observer->getEvent()->getCustomer()->getStoreId());
            }
        }
    }
