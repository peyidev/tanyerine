...
class J2t_Rewardpoints_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function pointsListAction()
    {
        ...
        $filename = 'points.csv';
        //$filename = 'points.pdf';
        $content = Mage::helper('rewardpoints')->generatePointsList($from_date, $end_date);
        $this->_prepareDownloadResponse($filename, $content);
    }
    
    public function pointslistprintAction()
    {
        $data = array();
        if ($this->getRequest()->getParam('start')){
            $data['from_date'] = $this->getRequest()->getParam('start');
        }
        if ($this->getRequest()->getParam('to')){
            $data['to_date'] = $this->getRequest()->getParam('to');
        }
    ...
    public function preDispatch()
    {
        ...
        $actions_array = array('referral', 'points', 'pointsList', 'pointslistprint');
