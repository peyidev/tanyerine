...
class J2t_Rewardpoints_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getProductPointsText($product, $noCeil = false, $from_list = false, $customer_group_id = null) {
        ...
        if ($points && $product->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_BUNDLE && $product->getPriceType() == Mage_Bundle_Model_Product_Price::PRICE_TYPE_FIXED) {
            ...
            $return = '<p class="j2t-loyalty-points inline-points">' . $img . Mage::helper('rewardpoints')->__("<div class='reward-flag'><span id='j2t-pts'>%s</span> PUNTOS", $points) . '</div></p>';
            return $return;
        } else if ($from_list) {
    ...
    public function generatePointsList($from_date = null, $end_date = null) {
        
        //$content    = $magento_block->createBlock('cdiscountimport/adminhtml_sales_order_grid')->setOrderId($order_id)->removeColumn('store_id');
        $magento_block = Mage::getSingleton('core/layout');
        $content    = $magento_block->createBlock('rewardpoints/adminhtml_clientpoints_grid')
                        ->setCustomerId(Mage::getSingleton('customer/session')->getCustomer()->getId())
                        ->setCSVExport(true)
                        ->sortColumnsByOrder();
        
        if ($from_date) {
            $content->setFromDate($from_date);
        }
        if ($end_date) {
            $content->setToDate($end_date);
        }
        
        $excel_file_content = $content->getCsvFile();
        //$excel_file_content = $content->getPdfFile();
        
        $exportReadyDir = Mage::getBaseDir('var') . DS . 'export' . DS;

        $file = new Varien_Io_File();
        //Create import_ready folder
        $importReadyDirResult = $file->mkdir($exportReadyDir);

        if (!$importReadyDirResult) {
            //Handle error
        }
        
        //remove columns (store_id, client_id, order_id)

        $file_name = $exportReadyDir.DS.md5(microtime());
        $file->write($file_name, $excel_file_content['value']);
        return array(
            'type'  => 'filename',
            'value' => $file_name,
    ...
    public function getPointsOnOrder($cartLoaded = null, $cartQuote = null, $specific_rate = null, $exclude_rules = false, $storeId = false, $money_points = false) {
        ...
        } else if (!is_object($object_tovalidate) && is_object(Mage::getModel('checkout/cart')->getQuote())) {
            $object_tovalidate = Mage::getModel('checkout/cart')->getQuote();
        }
