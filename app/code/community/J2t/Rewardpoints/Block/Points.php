...
{
    const XML_PATH_POINT_COUPON          = 'rewardpoints/j2trewardcouponlink/point_coupon';
    ...
    public function getAvailableRules() {
        $points_coupon = Mage::getStoreConfig(self::XML_PATH_POINT_COUPON);
        $points_coupon_array = unserialize($points_coupon);
        //$return = array();
        if (sizeof($points_coupon_array)){
            $level = Mage::helper('j2trewardcouponlink')->getCurrentCustomerLevel();
            $i = 0;
            foreach ($points_coupon_array as $point_coupon){
                if (isset($point_coupon['point_value']) && isset($point_coupon['rule_id']) && isset($point_coupon['template'])
                ){
                    $i++;
                    //TODO >> verify level (1, 2, 3, etc.) in order to show the information
                    if ($i > $level){
                        $rule_id = $point_coupon['rule_id'];
                        $rule = Mage::getModel('salesrule/rule')->load($point_coupon['rule_id']);
                        $name = $this->__('%s points: %s', $point_coupon['point_value'], $rule->getName());
                        return array('rule_id' => $rule_id, 'rule_name' => $rule->getName(), 'rule_description' => $rule->getDescription(), 'required_points' => $point_coupon['point_value']);
                    }
                }
            }
        }
    ...
    public function getLevelPoints() {
        return Mage::helper('j2trewardcouponlink')->getLevelPoints();
    }
    
    public function isCouponLevels() {
        return (bool)Mage::getStoreConfig('rewardpoints/j2trewardcouponlink/keep_point');
    }
